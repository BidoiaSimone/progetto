...
int MS;
	...
	void way_printer(char **M, int *row, int *col, int start_row, int start_col, string_t *moves){
		vector_t *way = v_create();
		v_push_back(way, start_row, start_col);

		for(int i = 0; i < strlen(moves->string); i++){
			if(moves->string[i] == 'N'){
				v_push_back(way, start_row-1, start_col);
				start_row--;
			}
			if(moves->string[i] == 'S'){
				v_push_back(way, start_row+1, start_col);
			...
			if(moves->string[i] == 'E'){
				v_push_back(way, start_row, start_col+1);
				start_col++;
			}
			if(moves->string[i] == 'O'){
				v_push_back(way, start_row, start_col-1);
...
int main(){
    ...
    if(selection == 2){           //--challenge branch (IA)
		...
		way_printer(M, &row, &col, g_row, g_col, moves);
		s_print(moves);
		s_free(moves);
